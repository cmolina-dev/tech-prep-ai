    ...
    >
      ...
      <div className="flex-1 flex flex-col p-6 pt-4">
        <div className="flex items-start justify-between gap-2 mb-3">
          ...
          {isSelected && (
            <div className="w-6 h-6 flex items-center justify-center bg-blue-500 rounded-full text-white shrink-0 shadow-lg shadow-blue-500/40 animate-in zoom-in duration-200">
        ...
        <div className="mt-auto">
          <div className="flex flex-wrap gap-2">
            {path.topics.slice(0, 3).map((topic, index) => (
              <span key={index} className="px-2.5 py-1 bg-white/5 border border-white/5 rounded-md text-[11px] font-semibold text-slate-400 uppercase tracking-wide">
                {topic}
              </span>
            ))}
          </div>
