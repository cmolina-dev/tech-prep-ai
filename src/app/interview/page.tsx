...
import { Settings, User, Mic, Trash2, RefreshCw, Send } from "lucide-react";
    ...
    <div className="h-screen flex flex-col bg-background text-foreground overflow-hidden font-sans">
      ...
      <div className="h-16 flex items-center justify-between px-6 bg-secondary border-b border-border shrink-0">
        <Link href="/" className="no-underline">
          <div className="flex items-center gap-4">
            <div className="w-8 h-8 bg-primary rounded-md flex items-center justify-center text-xl">
              💻
            </div>
            <div className="flex flex-col">
              <h1 className="text-base font-bold leading-tight m-0">
                TechPrepAI
              </h1>
        ...
        <div className="flex bg-background p-1 rounded-md gap-0.5">
          <button className="px-4 py-1.5 rounded-sm bg-blue-500/10 text-blue-500 text-sm font-medium border-none cursor-pointer transition-all duration-200">
            Practice
          </button>
          <button className="px-4 py-1.5 rounded-sm bg-transparent text-secondary-foreground text-sm font-medium border-none cursor-pointer transition-all duration-200 hover:bg-white/5">
            History
          </button>
          <button className="px-4 py-1.5 rounded-sm bg-transparent text-secondary-foreground text-sm font-medium border-none cursor-pointer transition-all duration-200 hover:bg-white/5">
            Stats
        ...
        <div className="flex items-center gap-4">
          <button
            onClick={() =>
              sessionId && router.push(`/summary?sessionId=${sessionId}`)
            }
            style={{
              marginRight: "1rem",
              padding: "0.5rem 1rem",
      ...
      <div className="p-4 flex flex-col items-center gap-2 bg-background">
        <div className="flex items-center gap-4 bg-card px-4 py-2 rounded-full border border-border">
          <div
            className={`flex items-center gap-1.5 text-sm font-medium transition-all ${status === "listening" ? "opacity-100 text-red-500 bg-red-500/10 px-2 py-[2px] rounded-sm" : "opacity-60 text-muted-foreground"}`}
          >
            <span className="w-2 h-2 bg-red-500 rounded-full shadow-[0_0_8px_#ef4444]"></span>{" "}
            LISTENING
          </div>
          <div
            className={`flex items-center gap-1.5 text-sm font-medium transition-all ${status === "thinking" ? "opacity-100 text-foreground" : "opacity-60 text-muted-foreground"}`}
          >
            <span className="">💭</span> Thinking
          </div>
          <div
            className={`flex items-center gap-1.5 text-sm font-medium transition-all ${status === "speaking" ? "opacity-100 text-foreground" : "opacity-60 text-muted-foreground"}`}
          >
      ...
      {/* Chat Area */}
      <div className="flex-1 overflow-y-auto px-[10%] lg:px-[20%] py-6 scroll-smooth">
      ...
      {/* Input Area */}
      <div className="bg-background px-[10%] lg:px-[20%] py-6 flex flex-col gap-6 z-10 border-t border-white/5 shrink-0">
        {/* Listening Text */}
        {status === "listening" && (
        ...
        <div className="flex items-center bg-card px-6 py-4 rounded-lg border border-border gap-4">
          ...
          <input
            ...
            placeholder={
              status === "listening" ? "Listening..." : "Type your answer..."
            }
            value={inputValue}
          ...
          />
          <button
            className="bg-transparent border-none text-muted-foreground cursor-pointer text-lg flex items-center justify-center"
        ...
        <div className="flex items-center justify-center gap-12 relative">
          ...
          <div className="flex flex-col items-center gap-1">
            <button
              className="w-12 h-12 rounded-full bg-card border-none text-secondary-foreground text-xl flex items-center justify-center cursor-pointer transition-all duration-200 hover:bg-muted hover:text-foreground"
